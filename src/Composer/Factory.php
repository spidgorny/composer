...
class Factory
{
	...
	public function createComposer(IOInterface $io, $localConfig = null, $disablePlugins = false, $cwd = null, $fullLoad = true)
	{
		...
		if (is_string($localConfig)) {
			$composerFile = str_replace('.\\', '', $localConfig);

			$file = new JsonFile($localConfig, null, $io);

			if (!$file->exists()) {
				clearstatcache(true);
				$io->writeError('cwd: ' . getcwd());
				$io->writeError('$file: ' . $file->getPath());
				$io->writeError('realpath: ' . realpath($file->getPath()));
//				$io->writeError('stat: ' . json_encode(stat($file->getPath()), JSON_PRETTY_PRINT));
				$io->writeError('exists: ' . file_exists($file->getPath()));
